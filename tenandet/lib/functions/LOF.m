...
% modified further by: Seyyid Emre Sofuoglu(sofuoglu@msu.edu)
%                                                                   
%                                                                   
% Inputs                                                            
...
k_index = cellfun(@(x) x(2:end),k_index,'UniformOutput',false); %min(1+k,length(x))
...
for i = 1:n
    lrd_value(i) = lrd(A, i, k_dist1, k_index, numneigh(i))+eps;
end
...
Temp = A(index_p,:) - A(k_index{index_p}, :);
Temp = sqrt(sum(Temp.^2,2));
reach_dist = max([Temp k_dist(k_index{index_p})],[],2);
%Calculate the local reachability density for each element
reach_dist = max(sum(reach_dist), norm(A(index_p,:))^2+0.1*numneighbors);
lrd_value = numneighbors/max((reach_dist),eps);
end
