...
def lr_stss_incomplete(Y, A, temp_m, spat_m, **kwargs):
    ...
    Lda_t = np.zeros(sz)                        # W_t = W x_t Delta
    Lda_loc = np.zeros(sz)                      # W_loc = W x_l Ln
    Lda = np.zeros(sz)                          # W = S
    ...
    while it<max_it:
        ...
        # W_t update
        W_t = soft_treshold(m2t(Delt@t2m(W_t, temp_m), sz, temp_m)+Lda_t/rho,
                            lda_t/rho)
        # W_loc update
        W_loc = soft_treshold(m2t(Delt@t2m(W_t, spat_m), sz, spat_m)+Lda_loc/rho,
                            lda_loc/rho)
        # S update
        sobs = (lda_2*(Y[obs_idx]-X[obs_idx]) + rho*(W[obs_idx]+Lda[obs_idx]/rho))/(lda_2+rho)
        S[obs_idx] = soft_treshold(sobs, lda_1/(lda_2+rho))
        S[unobs_idx] = soft_treshold(W[unobs_idx]+Lda[unobs_idx]/rho, lda_1/rho)

        # {W, X} Block updates
        # X update
        sumx = sum(X_i) + sum(Lda_i)/rho
        X[obs_idx] = (rho*sumx[obs_idx] + lda_2*(Y[obs_idx] - S[obs_idx]))(N*rho+lda_2)
        X[unobs_idx] = sumx[unobs_idx]/N
